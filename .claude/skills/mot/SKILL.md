...
if [ -f ".claude/settings.json" ]; then
  ...
  echo "FAIL: settings.json not found (hooks won't fire)"
fi

# Check skill-rules.json exists and is valid
if [ -f ".claude/skills/skill-rules.json" ]; then
  echo "PASS: skill-rules.json exists"
  SKILL_COUNT=$(python3 -c "import json; d=json.load(open('.claude/skills/skill-rules.json')); print(len(d.get('skills',{})))" 2>/dev/null)
  echo "INFO: $SKILL_COUNT skills registered in skill-rules.json"
else
...
3. **Create missing cache directories**
   ```bash
   mkdir -p .claude/cache/agents/{scout,kraken,spark}
